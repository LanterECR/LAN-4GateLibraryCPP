    ...
    namespace Message {
        namespace Response {
            ...
            public:
                ...
                /// \brief Возвращает общую сумму продаж при сверке итогов
                /// \return Если поле установлено - значение суммы операции в диапазоне [0, 999999999999]. Иначе -1.
                virtual int64_t getSalesAmount() const = 0;

                /// \brief Устанавливает общую сумму продаж при сверке итогов в минимальных единицах валюты (копейки для рублей)
                /// \param[in] totalAmount Значение суммы в диапазоне [0, 999999999999]
                /// \return true, если поле успешно установлено
                virtual bool setSalesAmount(int64_t amount) = 0;

                /// \brief Устанавливает значение по умолчанию для поля SalesAmount
                /// и удаляет из списка установленных полей
                /// \return true, если успешно удалено.
                virtual bool resetSalesAmount() = 0;
                ...
                /// \brief Возвращает общую сумму отмен при сверке итогов
                /// \return Если поле установлено - значение суммы операции в диапазоне [0, 999999999999]. Иначе -1.
                virtual int64_t getVoidAmount() const = 0;

                /// \brief Устанавливает общую сумму отмен при сверке итогов в минимальных единицах валюты (копейки для рублей)
                /// \param[in] totalAmount Значение суммы в диапазоне [0, 999999999999]
                /// \return true, если поле успешно установлено
                virtual bool setVoidAmount(int64_t amount) = 0;

                /// \brief Устанавливает значение по умолчанию для поля VoidAmount
                /// и удаляет из списка установленных полей
                /// \return true, если успешно удалено.
                virtual bool resetVoidAmount() = 0;
                ...
                /// \brief Возвращает общую сумму возвратов при сверке итогов
                /// \return Если поле установлено - значение суммы операции в диапазоне [0, 999999999999]. Иначе -1.
                virtual int64_t getRefundAmount() const = 0;

                /// \brief Устанавливает общую сумму возвратов при сверке итогов в минимальных единицах валюты (копейки для рублей)
                /// \param[in] totalAmount Значение суммы в диапазоне [0, 999999999999]
                /// \return true, если поле успешно установлено
                virtual bool setRefundAmount(int64_t amount) = 0;

                /// \brief Устанавливает значение по умолчанию для поля VoidAmount
                /// и удаляет из списка установленных полей
                /// \return true, если успешно удалено.
                virtual bool resetRefundAmount() = 0;
